...
### Decomposition
It basically states that every vector $\mathbf{y}$ in $\mathbb{R}^n$ can be ***uniquely*** written as the sum of a *vector* in a *subspace* $W$ of $\mathbb{R}^n$ and a *vector* in the *orthogonal complement* $W^\perp$ to $W$.

It can be written in the form:
$$
\mathbf{y} = \hat{\mathbf{y}}+\mathbf{z}
$$
Where $\hat{\mathbf{y}}$ is the ***[[Projection on Higher-Dimensional Subspaces|orthogonal projection]]*** of $\mathbf{y}$ onto the *subspace* $W$ and $\mathbf{z}$ is in $W^\perp$.
$$
