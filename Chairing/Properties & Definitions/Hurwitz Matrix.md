#Chairing #Definition 
A ***Hurwitz*** (or stable) ***matrix*** is a **square** matrix whose **eigenvalues** all have *strictly* negative *real* parts. For a $n\times n$ matrix:
$$
\lambda_{i} \text{ satisfy } \mathbb{R}(\lambda_{i}) < 0
$$
This implies that solutions to $\dot{x}=Ax$ decay exponentially to zero. We also note that a matrix $A$ is ***Hurwitz if and only if*** there is a symmetric [[Positive Definite Matrix]] $P$ satisfying the ***Lyapunov equation***:
$$
A^TP+PA=-Q, \text{ } Q>0
$$
$$\boxed{\text{A matrix } A \text{ is Hurwitz if all its eigenvalues have negative real parts.}}$$

